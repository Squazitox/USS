...
$('#btnBuscar').on('click', function () {
    ...
    jQuery.ajax({
        url: $.MisUrls.url._ObtenerVencimiento + "?fechainicio=" + $("#txtFechaInicio").val() + "&fechafin=" + $("#txtFechaFin").val(),
        type: "GET",
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        success: function (data) {
            ...
            if (data != undefined && data != null) {
                ...
                $.each(data, function (i, row) {
                    ...
                    $("<tr>").append(
                        $("<td>").text(row["codigo"]),
                        $("<td>").text(row["Nombre"]),
                        $("<td>").text(row["Caducidad"]),
                        $("<td>").text(row["Cantidad"]),
                        $("<td>").text(row["Ubicacion"]),
...
function printData() {
    ...
    newWin.document.write("<h3>Reporte de Vencimiento</h3>");
